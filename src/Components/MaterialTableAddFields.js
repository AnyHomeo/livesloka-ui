...
const DropdownEditor = ({ onChange, value }) => {
	...
	return (
		<Autocomplete
			...
			renderInput={(params) => <TextField {...params} label={'Subjects'} variant="standard" margin="dense" />}
	...
	useEffect(() => {
		if (data.length) {
			...
			let v = Object.keys(data[lengths.indexOf(Math.max(...lengths))]).map((key) => {
				...
				if (key === 'zoomJwt' || key === 'zoomSecret' || key === 'zoomApi' || key === 'summerCampDescription' || key === "teacherImageLink" || key == "summerCampImageLink" ) {
					return {
				...
				if (key === 'timeSlots' || key === 'id' || key === '_id' || key === 'statusId' || key === 'tableData') {
