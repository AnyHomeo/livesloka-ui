...
import {getAllSchedulesByZoomAccountId, getData} from "../../Services/Services"
const times = [
	"12:00 AM-12:30 AM",
	"12:30 AM-01:00 AM",
	"01:00 AM-01:30 AM",
...
function MeetingDashboard() {
	...
	const [selectedZoomAccountSchedules, setSelectedZoomAccountSchedules] = useState([])
	...
	return (
		<>
			...
			<Autocomplete
				...
				getOptionLabel={(option) => `${option.ZoomAccountName}(${option.zoomEmail})`}
				onChange={(e, v) => {
				...
				renderInput={(params) => (
					<TextField {...params} label="Select Zoom Account" variant="outlined" margin="normal" />
				)}
			/>
			<div
				style={{
											...
											<div
												...
												style={{
													...
													backgroundColor:
														...
														selectedZoomAccount.timeSlots.includes(`${day.toUpperCase()}-${time}`)
															? "#B73427"
															: "",
												}}
											>
												...
												selectedZoomAccount.timeSlots.includes(`${day.toUpperCase()}-${time}`)
													? selectedZoomAccountSchedules.filter((schedule) =>
															schedule.slots[day].includes(`${day.toUpperCase()}-${time}`)
													  )[0]
														? selectedZoomAccountSchedules.filter((schedule) =>
																schedule.slots[day].includes(`${day.toUpperCase()}-${time}`)
														  )[0].className
