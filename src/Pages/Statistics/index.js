...
import {Grid, Card, CardActions, IconButton, Tooltip} from "@material-ui/core/"
	...
	useEffect(() => {
		socket.on("teacher-joined", ({scheduleId}) => {
			setStatisticsData((prev) => {
				...
				tempStatisticsData = {
					...tempStatisticsData,
					schedulesRightNow: tempStatisticsData.schedulesRightNow.map((schedule) => ({
						...schedule,
						isTeacherJoined: schedule._id === scheduleId ? true : schedule.isTeacherJoined,
					})),
				}
									...
									<Grid item xs={6} sm={3}>
										<Card
											...
											style={{
												height: "100%",
												padding: "10px",
												backgroundColor: data.isTeacherJoined ? "#2ecc71" : "#f39c12",
												paddingBottom: "40px",
												position: "relative",
											}}
										>
											...
											>
												...
												<Link to={`/edit-schedule/${data._id}?goto=/statistics`}>
