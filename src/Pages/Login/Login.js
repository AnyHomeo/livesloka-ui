...
const Login = () => {
	...
	const onChange = (e) => setUser({...user, errors: false, [e.target.name]: e.target.value})

	const rememberChange = () => setUser({...user, errors: false, remember: !remember})
	...
	const onSubmit = (event) => {
		...
		login(userId, password)
			.then((data) => {
				console.log(data)
				localStorage.setItem("roleID", data.data.result.token && data.data.result.roleId)
				if (
	...
	const performRedirect = () => {
		console.log(isAutheticated().roleId)
		if (isAutheticated()) {
			...
			} else if (isAutheticated() && isAutheticated().AgentDesc === "Accountants") {
				return <Redirect to="/accountant/invoice" />
			} else if (
			...
			<Grid item xs={false} sm={4} md={7} className={classes.image}>
				<div className={classes.ImageGroup}>
					...
					<img
						...
						alt="qr code"
					/>
					<Button className={classes.image1} type="submit" variant="contained" color="primary">
						QR CODE
					</Button>
				</div>
			</Grid>
