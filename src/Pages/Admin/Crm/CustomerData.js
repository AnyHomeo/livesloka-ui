...
const getSlotFromTime = (date) => {
	...
	let secondsLeft = (is30 ? 59 - minutesRightNow : 29 - minutesRightNow) * 60 + (60 - secondsRightNow);
...
const names = ['Class', 'Time Zone', 'Class Status', 'Currency', 'Country', 'Teacher', 'Agent', 'Category', 'Subject'];
...
const ColumnFilterDrawer = ({ drawerOpen, setDrawerOpen, columnFilters, setColumnFilters, classes }) => (
	<Drawer
		anchor={'right'}
		open={drawerOpen}
		onClose={() => {
		...
		if (Object.keys(columnFilters).length) {
			setColumns([
				...
				},
				{
					...
					field: 'paymentDate',
					width: '1%',
					lookup: Array.from({ length: 31 }, (_, i) => i).reduce((prev, cur) => ({ ...prev, [cur]: cur })),
					cellStyle: { whiteSpace: 'nowrap' },
					headerStyle: { whiteSpace: 'nowrap' },
	...
	const AutoCompleteFilterData = ({ dropdown, i }) => {
		return (
			<Autocomplete
				...
				value={
					filters[['classStatuses', 'timeZones', 'classes', 'teachers', 'countries', 'agents', 'subjects'][i]]
				}
				onChange={(e, v) => {
					setFilters((prev) => {
						...
						return {
							...prevFilters,
							[['classStatuses', 'timeZones', 'classes', 'teachers', 'countries', 'agents', 'subjects'][
								i
							]]: v,
						};
					});
				...
				renderInput={(params) => (
					<TextField
						{...params}
						variant="outlined"
						label={['Class Status', 'Time Zone', 'Class', 'Teacher', 'Country', 'Agent', 'Subject'][i]}
					/>
				)}
			/>
		);
		...
		<>
						...
						<Grid container style={{ margin: '0 auto', width: '90%' }}>
							<Grid item xs={12} md={6}>
								...
								<Grid container style={{ display: 'flex', flexDirection: 'row' }}>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#e74c3c' }}>
											<h2>{'<-2'}</h2>
											<h1 style={{ color: 'white' }}>{statisticsData.customersLessThanMinus2}</h1>
										</Card>
									</Grid>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#e67e22' }}>
											<h2>{'<-1'}</h2>
											<h1 style={{ color: 'white' }}>{statisticsData.customersEqualToMinus1}</h1>
										</Card>
									</Grid>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#2ecc71' }}>
											<h2>{'0'}</h2>
							...
							<Grid item xs={12} md={6}>
								...
								<Grid container style={{ display: 'flex', flexDirection: 'row' }}>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#d35400' }}>
											<h2>New</h2>
											<h1 style={{ color: 'white' }}>{statisticsData.newCustomers}</h1>
										</Card>
									</Grid>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#3498db' }}>
											<h2>Demo</h2>
											<h1 style={{ color: 'white' }}>{statisticsData.demoCustomers}</h1>
										</Card>
									</Grid>
									<Grid item xs={6} sm={4}>
										<Card className={classes.card} style={{ backgroundColor: '#27ae60' }}>
											<h2>InClass</h2>
			...
			{historyStudentData && (
				...
				>
					...
					<StudentHistoryTable data={historyStudentData} id={historySelectedId} />
				</Dialog>
			)}
		</>
	);
