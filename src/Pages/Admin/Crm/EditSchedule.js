...
const EditSchedule = () => {
	...
	const getTeachers = async () => {
		const teacherNames = await Axios.get(`${process.env.REACT_APP_API_KEY}/teacher?params=id,TeacherName`);
	...
	const getScheduleData = async () => {
		try {
			...
			setTimeSlotState([...monday, ...tuesday, ...wednesday, ...thursday, ...friday, ...saturday, ...sunday]);
			setPrevSlots([...monday, ...tuesday, ...wednesday, ...thursday, ...friday, ...saturday, ...sunday]);
		} catch (error) {
			console.log(error);
		}
	};
						...
						>
							...
							>
								{zoomAccounts.map((account) => (
									<MenuItem value={account._id}>{account.ZoomAccountName}</MenuItem>
								))}
							</Select>
						</FormControl>
						<TextField
