...
import {getAllTeachers, getAllStudents, postMeeting} from "../../../Services/Services"
...
const Admin = () => {
	...
	useEffect(() => {
		getAllStudents().then((data) =>
			setStudents(data.data.map((name) => `${name.STUDENT_NAME}(${name.STUDENT_ID})`))
		)
		getAllTeachers().then((data) =>
			setTeachers(data.data.map((name) => `${name.TEACHER_NAME}(${name.TEACHER_ID})`))
		)
	}, [])

	const submitForm = (e) => {
		e.preventDefault()
		postMeeting(teacher, student, startDate, endDate, startTime, endTime, day).then((data) => {
				...
				<div className="textgroup">
					...
					<Autocomplete
						...
						onChange={(event, value) =>
							value &&
							setInputTeacher(value.split("(")[1].substring(0, value.split("(")[1].length - 1))
						}
						options={teachers}
						renderInput={(params) => (
							<TextField
					...
					<Autocomplete
						...
						onChange={(event, value) =>
							value &&
							setInputStudent(value.split("(")[1].substring(0, value.split("(")[1].length - 1))
						}
						options={students}
						renderInput={(params) => (
							<TextField
