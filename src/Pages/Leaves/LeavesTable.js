...
function LeavesTable() {
	...
	useEffect(() => {
		if (Object.keys(selectedCustomer).length && selectedDateToCancel) {
			Axios.get(
				`${process.env.REACT_APP_API_KEY}/cancelclass/start/end?date=${moment(selectedDateToCancel).format(
					'YYYY-MM-DD'
				)}&agent=${isAutheticated().agentId}&customerId=${selectedCustomer._id}`
			)
				.then((data) => {
